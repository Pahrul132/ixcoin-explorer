...
export default function Mining() {
  ...
  const nextReward = stats?.blockReward ?? 10;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        <h2 className="font-semibold mb-1">Jadwal Halving IXCOIN Network</h2>
        <p className="text-xs text-muted-foreground mb-4">Reward miner berkurang 50% setiap ~3.8 tahun (400.000 blok)</p>
        <div className="space-y-3">
          {[
            { era: 1,  startBlock: 0,         endBlock: 400_000,   blocks: "0 – 400.000",           period: "~Tahun 0–3.8",   reward: 10,      rewardStr: "10 IXC" },
            { era: 2,  startBlock: 400_000,   endBlock: 800_000,   blocks: "400.001 – 800.000",      period: "~Tahun 3.8–7.6", reward: 5,       rewardStr: "5 IXC" },
            { era: 3,  startBlock: 800_000,   endBlock: 1_200_000, blocks: "800.001 – 1.200.000",    period: "~Tahun 7.6–11.4", reward: 2.5,    rewardStr: "2.5 IXC" },
            { era: 4,  startBlock: 1_200_000, endBlock: 1_600_000, blocks: "1.200.001 – 1.600.000",  period: "~Tahun 11.4–15.2", reward: 1.25,  rewardStr: "1.25 IXC" },
            { era: 5,  startBlock: 1_600_000, endBlock: 2_000_000, blocks: "1.600.001 – 2.000.000",  period: "~Tahun 15.2–19", reward: 0.625,   rewardStr: "0.625 IXC" },
            { era: 6,  startBlock: 2_000_000, endBlock: 2_400_000, blocks: "2.000.001 – 2.400.000",  period: "~Tahun 19–22.8", reward: 0.3125,  rewardStr: "0.3125 IXC" },
            { era: 7,  startBlock: 2_400_000, endBlock: null,       blocks: "2.400.001+",             period: "~Tahun 22.8+ (berlanjut hingga 2140)", reward: 0, rewardStr: "→ 0 IXC" },
          ].map((row) => {
            const h = stats?.height ?? 0;
            const active = row.endBlock !== null
              ? h >= row.startBlock && h < row.endBlock
              : h >= row.startBlock;
            return (
              <div key={row.era} className={`flex items-center gap-4 rounded-lg p-3 border ${
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-mono text-muted-foreground truncate">{row.blocks}</p>
                  <p className="text-xs text-muted-foreground/60">{row.period}</p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className={`text-sm font-mono font-bold ${active ? "text-yellow-300" : "text-foreground"}`}>{row.rewardStr}</span>
        ...
        <p className="text-xs text-muted-foreground mt-3">
          Total supply mining: 8.000.000 IXC · Biaya Pengembangan: 13.000.000 IXC · Max: 21.000.000 IXC · Mining selesai ~2140
        </p>
      </div>
    </div>
  );
